...
// Impulse Force - used once collision is detected
ImpulseForce::ImpulseForce(const ofVec3f &vel, const ofVec3f &norm) {
    applyOnce = true;
    velocity = vel;
    normal = norm;
}
...
void ImpulseForce::updateForce(Particle * particle) {
    ...
    ofVec3f f = (1.2) * (glm::dot(-v, n)) * n;
    particle->forces += ofGetFrameRate() * f;
}

ThrusterForce::ThrusterForce(ofVec3f a) {
    acceleration = a;
}

void ThrusterForce::updateForce(Particle * particle) {
    particle->forces.x += acceleration.x * particle->mass;
    particle->forces.y += acceleration.y * particle->mass;
    particle->forces.z += acceleration.z * particle->mass;
}
